...
- BodyMesh v3 still accepts shoulder width, arm length, upper-arm circumference, thigh circumference and inseam. When the four A4 target views are usable, BodyCalibration v4 also stores a shared pinhole CameraRig with intrinsic parameters, per-view pose and reprojection diagnostics.
...
- The 3D mode uses Three.js WebGPURenderer with WebGL2 fallback, perspective, depth testing, ACES tone mapping, studio lighting, shadows, roughness and sheen. The four body photos are exposure-matched and feather-blended into one 360° body atlas before rendering. Garments can store measured density, thickness, warp/weft stretch, bend stiffness and friction; a reusable engineering fabric library can prefill these values.
