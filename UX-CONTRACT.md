...
- **Save body:** validate client-side and resolve all four local images. Apply the reusable optical profile when image aspect matches, segment full masks, and reconstruct a visual hull only under the fixed-camera/person-turntable contract. Persist BodyCalibration v6 when dense carving succeeds; otherwise preserve v5/v4/v3 fallbacks. Keep the previous saved profile untouched on failure.
...
- **Physical try-on:** ensure an anatomical BodyMesh v2 (upgrade legacy v1 calibration in memory when needed), use dense voxel body collision when v6 occupancy exists, build a fresh semantic runtime GarmentMesh, simulate locally with body + garment self-collision, then render the saved front/back garment textures over solved UV regions. Never mutate source garment media.
