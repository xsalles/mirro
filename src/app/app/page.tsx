...
export default function DashboardPage() {
  ...
  const bodyReady = Boolean(state.profile && Object.keys(state.profile.photos).length === 4);
  const garmentCount = state.garments.length;

  return (
    <div className="space-y-8">
      <div><h1 className="font-display text-4xl font-bold tracking-[-.045em] sm:text-5xl">Seu espelho começa aqui.</h1><p className="mt-3 max-w-2xl text-[var(--muted)]">Monte a base corporal, digitalize suas peças e depois combine tudo no provador.</p></div>
      <section className="grid gap-4 md:grid-cols-3" aria-label="Progresso de configuração">
        {[
          { href: "/app/body", icon: ScanLine, title: "Meu corpo", value: bodyReady ? "Pronto" : "Pendente", copy: "4 fotos + medidas", done: bodyReady },
          ...
          { href: "/app/try-on", icon: Sparkles, title: "Experimentar", value: bodyReady && garmentCount ? "Liberado" : "Aguardando", copy: "Corpo + pelo menos 1 peça", done: bodyReady && garmentCount > 0 },
      ...
      <aside className="rounded-2xl bg-[var(--ink)] p-6 text-white sm:p-8"><p className="max-w-2xl text-sm leading-6 text-white/70">Nesta versão, suas fotos e peças ficam somente no IndexedDB deste navegador. Nada é enviado para uma API ou nuvem.</p></aside>
    </div>
  );
}
