...
- MVS/TSDF runs in a dedicated Web Worker when available; Census Hamming uses a WASM i32.popcnt microkernel and v10 additionally loads an AssemblyScript-built SIMD module for ZNCC/TSDF numeric reductions, with deterministic JS fallbacks
...
The current **Físico 3D beta** now adds robust classical multi-view stereo on top of the eight-view scan: ZNCC + Census + gradient matching, a 1×/½/¼ image pyramid, bounded coarse-to-fine plane sweep, parabolic subpixel refinement, turntable bundle optimization with per-frame angle residuals and axis tilt, confidence/ambiguity rejection, cross-view depth consistency and conservative TSDF fusion. The Worker uses a denser depth grid than the main-thread fallback. When the saved optical profile matches the scan, MVS uses calibrated turntable perspective; otherwise it falls back to metric orthographic projection. The v7 SDF remains the physical cloth-collision envelope. It still should not be sold as exact photoreal fit: eight-view MVS can recover only texture-supported geometry inside the conservative silhouette envelope; textureless, specular, self-occluded or moving regions still fall back to the visual hull, and fabric-library values remain engineering presets unless the user supplies measurements. See `ARCHITECTURE.md`.
