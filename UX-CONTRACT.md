...
- **Save body:** validate client-side and resolve all eight local images for a v7/v8/v9 scan. Existing four-view profiles remain readable. V9 prefers a dedicated MVS Worker and may use the WASM Census microkernel; Worker/WASM absence must fall back deterministically rather than invalidate the scan. Insufficient texture, low confidence or inconsistent depths MUST preserve the v7 result rather than fail the body save. Apply the reusable optical profile when image aspect matches, segment full masks, and reconstruct a visual hull only under the fixed-camera/person-turntable contract. V7 requires 45° increments, computes an EDT signed-distance field and persists BodyCalibration v7 when all eight views succeed; otherwise preserve v6/v5/v4/v3 fallbacks. Keep the previous saved profile untouched on failure.
