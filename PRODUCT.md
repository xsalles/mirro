...
MIRRO is a local-first digital wardrobe and virtual try-on product. A person can first calibrate the phone lens with 6–12 target photos, then captures four fixed-camera body views (front, right, back, left), basic measurements, and front/back photos of garments. The product helps them preview combinations without physically changing clothes.
...
- BodyMesh still accepts shoulder width, arm length, upper-arm circumference, thigh circumference and inseam. BodyCalibration v6 can add a dense four-view visual hull from the full masks while retaining the anatomical mesh as a semantic/fallback model.
